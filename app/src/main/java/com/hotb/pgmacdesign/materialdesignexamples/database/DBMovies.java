package com.hotb.pgmacdesign.materialdesignexamples.database;
...
/**
 * Created by Patrick on 2015-05-07.
 */
public class DBMovies {
	 ...
	 * @param table the table cccccccccccccccccccccccccccccccccccccccccccccccc
	 * @param listMovies list of movies in an arraylist to insert
	 * @param clearPrevious Boolean, if true, it will delete the previous database files
	 */
	public void insertMovies(int table, ArrayList<Movie> listMovies, boolean clearPrevious) {
		...
		for (int i = 0; i < listMovies.size(); i++) {
			...
			statement.bindString(3, currentMovie.getReleaseDateTheater());
	...
	public ArrayList<Movie> readMovies(int table) {
		...
		if (cursor != null && cursor.moveToFirst()) {
			...
			do {
				...
				String releaseDateString = cursor.getString(cursor.getColumnIndex(MoviesHelper.COLUMN_RELEASE_DATE));
				//movie.setReleaseDateTheater(releaseDateMilliseconds != -1 ? new Date(releaseDateMilliseconds) : null);
				movie.setReleaseDateTheater(releaseDateString);
